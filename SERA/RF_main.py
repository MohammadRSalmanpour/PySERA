...
def SERA_FE_main_Fun(data_orginal,
    ...
    for m in range(0, Lbin):
        ROIBox3D, levels3D, ROIonlyMorph3D, IntsROI, RawInts, RawROI, newPixW, newSliceTh = prepareVolume(data_orginal,
                                                                                                          Data_ROI_mat,
                                                                                                          DataType,
                                                                                                          pixelW,
                                                                                                          sliceTh,
        ...
        if isIsot2D == 1 or pixelW == isotVoxSize or isScale == 0:
            ...
            ROIBox2D, levels2D, ROIonly2D, _, _, _, _, _ = prepareVolume(data_orginal, Data_ROI_mat, DataType, pixelW,
                                                                         sliceTh, isotVoxSize2D, VoxInterp, ROIInterp,
                                                                         ROI_PV, 'XYscale', 0, isScale, isGLround,
                                                                         DiscType, qntz, BinSize[m], isReSegRng,
                                                                         ReSegIntrvl, isOutliers)
        ...
        if MultiBin == 0:
            for i in range(0, len(FeaturesType)):
                ...
                if FeaturesType[i] == '1st':
                    ...
                    # GLDZM
                    GLDZM2D, GLDZM3D, GLDZM25D = getGLDZMtex(ROIBox2D, ROIBox3D, ROIonly2D, ROIonlyMorph3D, levels2D,
                                                             levels3D)
        ...
        Feats = list(
            itertools.chain(np.squeeze(MorphVect), np.squeeze(SUVpeak), np.squeeze(StatsVect), np.squeeze(HistVect),
                            np.squeeze(IVHvect), np.squeeze(
                    GLCM2D_KSKD), np.squeeze(GLCM2D_KSMD), np.squeeze(GLCM2D_MSKD), np.squeeze(GLCM2D_MSMD),
                            np.squeeze(GLCM3D_Avg), np.squeeze(GLCM3D_Cmb), np.squeeze(
                    GLRLM2D_KSKD), np.squeeze(GLRLM2D_KSMD), np.squeeze(GLRLM2D_MSKD), np.squeeze(GLRLM2D_MSMD),
                            np.squeeze(GLRLM3D_Avg), np.squeeze(GLRLM3D_Cmb), np.squeeze(
                    GLSZM2D), np.squeeze(GLSZM25D), np.squeeze(GLSZM3D), np.squeeze(GLDZM2D), np.squeeze(GLDZM25D),
                            np.squeeze(GLDZM3D), np.squeeze(
                    NGTDM2D), np.squeeze(NGTDM25D), np.squeeze(NGTDM3D), np.squeeze(NGLDM2D), np.squeeze(NGLDM25D),
                            np.squeeze(NGLDM3D), np.squeeze(
                    MI_feats)))
