...
from Sera_ReadWrite import readimage
...
data_orgina_PATH = r'E:\ibsi_phantom_data_sets-master\ibsi_1_digital_phantom\nifti\image\phantom.nii.gz'
Data_RO_PATH = r'E:\ibsi_phantom_data_sets-master\ibsi_1_digital_phantom\nifti\mask\mask.nii.gz'
# end section 1
...
ROI_PV = None  # (default 0.5) ROI partial volume threshold. Used to threshold ROI after resampling: i.e. ROI(ROI<ROI_PV) = 0, ROI(ROI>ROI_PV) = 1.
VoxInterp = None  # Image resampling interpolation type  ('nearest', 'linear', or 'cubic'). Note: 'cubic' yeilds inconsistensies with IBSI results.
ROIInterp = None  # ROI resampling interpolation type  ('nearest', 'linear', or 'cubic'), default: 'linear'
isotVoxSize = None  # New isotropic voxel size for resampling in 3D. This will be the new voxel size in X, Y and Z dimension.
isotVoxSize2D = None  # New voxel size for resampling slices in 2D. This maintains the Z dimension, and rescales both X and Y to this number.
isIsot2D = None  # (default 0) whether to resample image to isotropic 2D voxels (=1, i.e.keep the original slice thickness) or resample to isotropic 3D voxels (=0). (This is for 1st order features. Higher order 2D features are always calculated with original slice thickness).
...
ReSegIntrvl = None  # Range resegmentation interval. Intensity values outside this interval would be replaced by NaN.
...
qntz = None  # An extra option for FBN Discretization Type: Either 'Uniform' quantization or 'Lloyd' for Max-Lloyd quantization. (defualt: Uniform)
isQuntzStat = None  # (default 1) whether to use quantized image to calculate first order statistical features. If 0, no image resample/interp for calculating statistical features. (0 is preferrable for PET images)
...
IVH_Type = None  # Setting for Intensity Volume Histogram (IVH) Unit type={0: Definite(PET,CT), 1:Arbitrary(MRI,SPECT. This is FNB), 2: use 1000 bins, 3: use same discritization as histogram (for CT)}
IVH_DiscCont = None  # Disc/Cont = {0:Discrete(for CT), 1:Continuous(for CT,PET. This is FBS)},
IVH_binSize = None  # Bin size for Intensity Volumen Histogram in case choosing setting 1 for FNB, or setting 0 and either IVH_DiscCont options.
        ...
        if (dim01 == dim02):
            ...
            if len(dim01) == 3 & len(dim02) == 3:
                ...
                if data_orgina[2] == 'Nrrd':
                    VoxelSize = (data_orgina[1]['space directions'][0, 0], data_orgina[1]['space directions'][1, 1],
                                 data_orgina[1]['space directions'][2, 2])
                ...
                if Data_RO[2] == 'Nrrd':
                    VoxelSizelabel = (Data_RO[1]['space directions'][0, 0], Data_RO[1]['space directions'][1, 1],
                                      Data_RO[1]['space directions'][2, 2])
                if Data_RO[2] == 'Dicom':
                    VoxelSizelabel = Data_RO[1].GetSpacing()
                ...
                if VoxelSizelabel == VoxelSize:
                    ...
                    writer = pd.ExcelWriter(CSVfullpath,
